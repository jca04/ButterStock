...
const createEditResipe = async (req, res) => {
  try {
    let {id_receta , nombre_receta, descripcion, tipoPlato, cantidad_plato, ingredient, sub_receta , id_restaurant, imagen} = req.body.data.data;
    ...
    if (id_receta == ''){
     ...
     conn.query("INSERT INTO tbl_recetas (id_receta, nombre_receta, imagen, descripcion,cantidad_plato, activo, sub_receta, tipo_receta, id_restaurant ) VALUES(?,?,?,?,?,?,?,?,?)", [id_receta_new, nombre_receta,imagen,descripcion,cantidad_plato,1, 0, tipoPlato, id_restaurant], 
      (err, result) => {
        if (err){
                 ...
                 [arrSubRecetas], (err, result) => {
                  ...
                  if (result.affectedRows > 0){
                    ...
                    if (arrSubRecetas.length > 0){
                      ...
                      conn.query('UPDATE tbl_recetas SET sub_receta = 1 WHERE id_receta IN ("'+concatIdSub+'")', (err, result) => {
                        if (err){
                          return res.status(500).json({message: err});
                        }
      ...
      conn.query("UPDATE tbl_recetas SET nombre_receta = ?, imagen = ?, descripcion = ?, cantidad_plato = ? WHERE id_receta = ?",
          ...
          if (result.affectedRows != undefined && result.affectedRows > 0){
            ...
            //ingredientes 
              ...
              (err, result) => {
                ...
                conn.query("INSERT INTO tbl_ingredientes_receta(id_ingrediente_receta, id_ingrediente, cantidad_por_receta, unidad_medida_r, id_receta) VALUES ?", [mapIngredients], 
                (err, result) => {
        ...
        if (arrConcat.length > 0){
          ...
          conn.query('SELECT Ir.*, i.nombre_ingrediente  FROM tbl_ingredientes_receta AS Ir INNER JOIN tbl_ingredientes AS i ON Ir.id_ingrediente = i.id_ingrediente WHERE Ir.id_receta  IN ("'+parseData+'") && Ir.activo = 1;',
           ...
           (err, resultIn) => {
            ...
            conn.query('SELECT sub.*, rec.nombre_receta FROM tbl_sub_recetas AS sub INNER JOIN tbl_recetas AS rec ON rec.id_receta =  sub.id_receta WHERE sub.id_receta IN ("'+parseData+'") && sub.activo = 1', 
            (err, resultSub) => {
              if (err){
                return res.status(500).json({message: err})
              }
