...
const verifiedRestaurant = async (req, res) => {
  ...
  try{
   if (name){
    await conn.query("SELECT id_restaurant FROM tbl_restaurant WHERE nombre = ?", [name],
      (err, result) => {
        ...
        if (result.length > 0){
          res.status(200).json({messgae: "restaurant exist yet"});
        }else{
          res.status(200).json({messgae: "restaurant doesn´t exist"});
        }
      }
    );
   }
